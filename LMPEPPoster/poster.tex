...
\begin{document}
	...
	\begin{columns} % See Section 4.4
		...
		\block{Aberth}{
			...
			\begin{equation}
		    A(z_{j}^{(k)})= -\frac{\frac{p(z_{j}^{(k)})}{p^{'}(z_{j}^{(k)})}}{1-\frac{p(z_{j}^{(k)})}{p^{'}(z_{j}^{(k)})} \cdot \sum\limits_{\substack{i=1\\i\neq j}}^{m}\frac{1}{z_{j}^{(k)}-z_{i}^{(k)}}}
			\end{equation}
		...
		\block{Classical Laguerre}{
			...
			\end{split}
			\end{equation}
			Then the next approximation is defined by
			\begin{equation}\label{eq:lag2}
