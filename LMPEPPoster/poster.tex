...
\begin{document}
	...
	\begin{columns} % See Section 4.4
		...
		\column{0.333}
		\block{Aberth}{
			Let $p(\lambda)$ be a polynomial of degree $m$. Let $(z_{1},\ldots,z_{m})$ be approximations to the roots, $r_{1},\ldots,r_{m}$, of $p(\lambda)$.. For $1\leq j\leq m$, we define
			\begin{equation}
		    A(z_{j})= -\frac{\frac{p(z_{j})}{p^{'}(z_{j})}}{1-\frac{p(z_{j})}{p^{'}(z_{j})}\dot{\sum_{\substack{i=1\\i\neq j}}^{m}\frac{1}{z_{j}-z_{i}}}}
			\end{equation}
		...
		\block{Classical Laguerre}{
			...
			\begin{equation}
			\begin{split}
			G_{j}(\lambda)&=\frac{p^{'}(\lambda)}{p(\lambda)}, \\
			H_{j}(\lambda)&=G^{2}_{j}-\frac{p^{''}(\lambda)}{p(\lambda)}.
			\end{split}
			\end{equation}
			Then our next approximation is defined by
			\begin{equation}\label{eq:lag2}
			...
			where
			\begin{equation}
			L(z_{j})=\frac{m}{G_{j}(z_{j})\pm\sqrt{(m-1)(mH_{j}(z_{j})-G_{j}^{2}(z_{j}))}}
			\end{equation}
			and the $\pm$ is chosen to maximize the magnitude of the denominator. }
