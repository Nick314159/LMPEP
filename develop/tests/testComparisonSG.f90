...
deg=startDegree
OPEN(UNIT=1,FILE="results/testComparisonSG.csv")
WRITE(1,'(A)') 'Degree, DSLM Time, DSLM Berr, Pzeros Time, Pzeros Berr, SG Time, SG Berr'
...
DO WHILE(deg<=endDegree)
  ...
  DO it = 1, itmax
    ...
    DO i= 0, deg
      poly(i)=cmplx(p(i+1), 0.0D0, KIND(0.D0))
    ENDDO
    CALL system_clock(count_rate=clock_rate)
    CALL system_clock(count=clock_start)
    ...
    DEALLOCATE(poly, radius ,root, err)
		...
		!cmplx_roots_sg
		ALLOCATE(root(deg),poly(deg+1),er(deg),ei(deg))
  	DO i= 0, deg
    ...
    ENDDO
  	...
  	CALL system_clock(count=clock_start)
		...
		root = cmplx(er,ei,KIND(0.0D0))
  	CALL cmplx_roots_gen(root,poly,deg,.FALSE.,.TRUE.)
  	CALL system_clock(count=clock_stop)
    ...
    error(it, 3) = maxval(berr)
		!Deallocate cmplex_roots_sg
		DEALLOCATE(root, poly, er, ei)
		!Deallocate poly
