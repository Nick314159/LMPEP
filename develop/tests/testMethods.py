...
with open('results/testMethods.csv') as f:
    ...
    axarr[0].loglog(degree, dslm_time, 'b-o',  label='DSLM_Simul')
    axarr[0].loglog(degree, dslm1_time, 'k--', label='DSLM_Seque')
    axarr[0].loglog(degree, dsam_time, 'r-*', label='DSAM')
    axarr[0].set_ylabel('Seconds')
    axarr[0].set_xlabel('Degree')
    axarr[0].set_title('Elapsed Time')
    legend = axarr[0].legend(loc=0, shadow=True)
    axarr[1].loglog(degree, dslm_berr, 'b-o', label='DSLM_Simul')
    axarr[1].loglog(degree, dslm1_berr, 'k--', label='DSLM_Seque')
    axarr[1].loglog(degree, dsam_berr, 'r-*', label='DSAM')
