SUBROUTINE DLAQP3( m, n, nA, A, ldA, jpvtA, tauA, rankA, tol, INFO )

! Purpose
! =======

! To find the QR factorization, with column pivoting, of the n by n real
! matrix A, together with the rank of A.  A is factorized as
!
!    A = Q*R*(P**T),
!
! where Q is an n by n orthogonal matrix, R is an n by n upper triangular matrix
...
!                  together with the array TAU, represent the orthogonal matrix
...
! tauA  - (output) REAL(kind=wp) array, dimension (MIN(m,n))
...
!      .. Array Arguments ..
       ...
       REAL (KIND=wp), INTENT (OUT)      :: tauA(n)
...
IF( nA /= ZERO )THEN
   ...
   lwork = -1
   CALL DGEQP3( m, n, A, ldA, jpvtA, tauA, dummy, lwork, INFO )
   lwork = dummy(1)
   ...
   ! [QA,RA,PA] = qr(A);
   CALL DGEQP3( m, n, A, ldA, jpvtA, tauA, work, lwork, INFO )
